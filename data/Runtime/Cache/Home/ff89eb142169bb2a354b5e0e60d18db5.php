	...
	<div class="pl">	
		<div class="toptitle">
			...
			<div class="ts">
				...
				<div class="J_detailList l3 <?php if($_COOKIE['jobs_show_type']== '0' or ($_COOKIE['jobs_show_type']== null and C('qscms_jobs_list_show_type') == 1)): ?>select<?php endif; ?>" title="切换到详细列表"></div>
				<div class="J_detailList l4 <?php if($_COOKIE['jobs_show_type']== '1' or ($_COOKIE['jobs_show_type']== null and C('qscms_jobs_list_show_type') == 2)): ?>select<?php endif; ?>" title="切换到简易列表" show_type="1"></div>
			...
			<?php if(!empty($jobslist['list'])): if(is_array($jobslist['list'])): $i = 0; $__LIST__ = $jobslist['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$jobs): $mod = ($i % 2 );++$i;?><div class="J_jobsList yli" data-jid="<?php echo ($jobs["id"]); ?>">
						...
						<div class="detail" <?php if($_COOKIE['jobs_show_type']== 1 or ($_COOKIE['jobs_show_type']== null and C('qscms_jobs_list_show_type') == 2)): ?>style="display:none"<?php endif; ?>>
							...
							<div class="rbtn <?php if($_GET['lng']!= ''): ?>map-btn<?php endif; ?>">
								<?php if(($jobs['allowance_id']) == "0"): ?><div class="deliver J_applyForJob" data-batch="false" <?php if(C('qscms_rapid_registration_resume') == 1): ?>data-url="<?php echo U('AjaxPersonal/resume_apply');?>"<?php else: if(C('visitor')): ?>data-url="<?php echo U('AjaxPersonal/resume_apply');?>"<?php else: ?>data-url="<?php echo U('AjaxCommon/ajax_login');?>"<?php endif; endif; ?> ><?php if(in_array($jobs['id'],$jobslist['has_apply'])): ?>已投递<?php else: ?>投递简历<?php endif; ?></div>
							  	<?php else: ?>
				...
				<div class="listbtn">
					...
					<div class="td2">
						<div class="lbts J_applyForJob" data-batch="true" <?php if(C('qscms_rapid_registration_resume') == 1): ?>data-url="<?php echo U('AjaxPersonal/resume_apply');?>"<?php elseif(C('visitor')): ?>data-url="<?php echo U('AjaxPersonal/resume_apply');?>"<?php else: ?>data-url="<?php echo U('AjaxCommon/ajax_login');?>"<?php endif; ?> >申请职位</div>
...
<?php if($apply['Subsite'] and $subsite_val['s_id'] > 0): ?><input type="hidden" class="map-lng" value="<?php if($_GET['lng']== ''): echo (($subsite_val["s_map_center_x"] != "")?($subsite_val["s_map_center_x"]):C('qscms_map_center_x')); else: echo (_I($_GET['lng'])); endif; ?>">
    <input type="hidden" class="map-lat" value="<?php if($_GET['lat']== ''): echo (($subsite_val["s_map_center_y"] != "")?($subsite_val["s_map_center_y"]):C('qscms_map_center_y')); else: echo (_I($_GET['lat'])); endif; ?>">
<?php else: ?>
    <input type="hidden" class="map-lng" value="<?php if($_GET['lng']== ''): echo C('qscms_map_center_x'); else: echo (_I($_GET['lng'])); endif; ?>">
    <input type="hidden" class="map-lat" value="<?php if($_GET['lat']== ''): echo C('qscms_map_center_y'); else: echo (_I($_GET['lat'])); endif; ?>"><?php endif; ?>
...
<script type="text/javascript" src="<?php echo C('TPL_PUBLIC_DIR');?>/js/jquery.jobslist.js"></script>
...
<script type="text/javascript" src="<?php echo C('TPL_PUBLIC_DIR');?>/js/jquery.modal.search.js"></script>
