...
<head>
	...
	<script>
		var URL = '/index.php/admin/appeal',
			SELF = '/index.php?m=admin&amp;c=appeal&amp;a=index',
...
<div class="seltpye_y">
    ...
    <div class="ct">
        <div class="txt <?php if(($_GET['status']) != ""): ?>select<?php endif; ?>"><?php echo ((_I($_GET['status_cn']) != "")?(_I($_GET['status_cn'])):"不限"); ?></div>
    </div>
    <div class="downlist">
        <li url="<?php echo P(array('status'=>'','status_cn'=>'不限'));?>">不限</li>
        <li url="<?php echo P(array('status'=>'0','status_cn'=>'未处理'));?>">未处理<?php if($count): ?><span style="color: #ff0000;">(<?php echo ($count); ?>)</span><?php endif; ?></li>
        <li url="<?php echo P(array('status'=>'1','status_cn'=>'已处理'));?>">已处理</li>
    </div>
</div>
<div class="seltpye_y">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_blue">
            <div class="td" style=" width:10%;">
                <div class="left_padding striking">
                    ...
                    <?php echo (($vo["realname"] != "")?($vo["realname"]):'-'); ?>
                </div>
            </div>
            <div class="td" style=" width:10%;">
                <?php if($vo['status'] == 1): ?>已处理
                    <?php else: ?><span style="color: #ff0000;">未处理</span><?php endif; ?>
            </div>
            <div class="td vtip" style=" width:28%;" title="<?php echo str_replace(array('&lt;','&gt;','/','<','>','script'),'',nl2br($vo['description']));?>"><?php echo (($vo["description"] != "")?($vo["description"]):'-'); ?></div>
            <div class="td" style=" width:10%;"><?php echo (($vo["mobile"] != "")?($vo["mobile"]):'-'); ?></div>
            <div class="td" style=" width:15%;"><?php echo (($vo["email"] != "")?($vo["email"]):'-'); ?></div>
            <div class="td center" style=" width:15%;"><?php echo admin_date($vo['addtime']);?></div>
            <div class="td edit" style=" width:12%;">
                <?php if($vo['status'] == 0): ?><a href="javascript:;" class="send_email" parameter="id=<?php echo ($vo['id']); ?>&email=<?php echo ($vo['email']); ?>">发送邮件</a><?php endif; ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".send_email").click(function () {
            ...
            var url = "<?php echo U('Ajax/appeal_send_email');?>&" + param;
        ...
        $("#ButAudit").click(function () {
            ...
            if(data.length == 0){
                ...
                var url = "<?php echo U('Ajax/appeal_audit');?>";
