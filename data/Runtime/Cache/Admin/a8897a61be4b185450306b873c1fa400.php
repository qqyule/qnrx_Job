...
<head>
	...
	<script>
		var URL = '/index.php/admin/hotword',
			SELF = '/index.php?m=admin&amp;c=hotword&amp;a=index',
...
<form id="form1" name="form1" method="post" action="<?php echo U('hotword/delete');?>">
    <div class="list_th">
        ...
        <div class="td center" style="width:37%;">搜索次数</div>
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_black">
            <div class="td" style="width:50%;">
                <div class="left_padding striking">
                    <label> <input name="w_id[]" type="checkbox" value="<?php echo ($vo["w_id"]); ?>"/><?php echo ($vo["w_word"]); ?></label>
                </div>
            </div>
            <div class="td center" style="width:37%;"><?php echo ($vo["w_hot"]); ?></div>
            <div class="td edit" style="width:13%;">
                <a href="<?php echo U('hotword/edit',array('w_id'=>$vo['w_id']));?>">修改</a>
                <a href="<?php echo U('hotword/delete',array('w_id'=>$vo['w_id']));?>" class="gray" onclick="return confirm('你确定要删除吗？')">删除</a>
            </div>
            <div class="clear"></div>
    ...
    <div class="list_foot">
        <div class="btnbox">
            <input id="ButAudit" type="button" class="admin_submit" value="添加词语" onclick="window.location='<?php echo U('hotword/add');?>'"/>
            <input id="ButDel" type="button" class="admin_submit" value="删除所选"/>
        </div>
    </div>
</form>
