...
<head>
	...
	<script>
		var URL = '/index.php/admin/payment',
			SELF = '/index.php?m=admin&amp;c=payment&amp;a=index',
...
<div class="toptip">
    ...
    <p>安装在线支付插件需申请对应在线支付服务公司的服务账户。</p>
</div>
<div class="list_th">
    <div class="td" style=" width:20%;"><div class="left_padding">名称</div></div>
    <div class="td" style=" width:50%;">支付方式简短描述</div>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_black">
    <div class="td" style=" width:20%;"><div class="left_padding" style="color:#3366CC"><?php echo ($vo["byname"]); ?></div></div>
    <div class="td" style=" width:50%;"><?php echo ($vo["p_introduction"]); ?></div>
    <div class="td center" style="width:15%;"><?php echo ($vo["listorder"]); ?></div>
    <div class="td edit" style=" width:15%;">
        <?php if($vo['p_install'] == '2'): ?><a href="<?php echo U('edit',array('id'=>$vo['id'],'typename'=>$vo['typename']));?>" >编辑</a>
        <?php else: ?>
