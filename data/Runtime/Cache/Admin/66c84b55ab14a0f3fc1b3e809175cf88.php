...
<head>
	...
	<script>
		var URL = '/index.php/admin/mail_queue',
			SELF = '/index.php?m=admin&amp;c=mail_queue&amp;a=index',
...
<form id="form1" name="form1" method="post" action="<?php echo U('del');?>">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_black">
        ...
        <div class="td" style=" width:15%;"><?php echo ($vo["m_mail"]); ?></div>
        <div class="td vtip" title="<?php echo (nl2br($vo['m_subject'])); ?>" style=" width:10%;"><?php echo ($vo['m_subject_']); ?></div>
        <div class="td vtip" title="<?php echo (nl2br($vo['m_body'])); ?>" style=" width:35%;"><?php echo ($vo['m_body_']); ?></div>
        <div class="td center" style=" width:10%;"><?php echo admin_date($vo['m_addtime']);?></div>
        <div class="td center" style=" width:10%;">
            <?php if($vo['m_sendtime'] == 0): ?>----
            <?php else: ?>
            <?php echo admin_date($vo['m_sendtime']); endif; ?>
        </div>
        <div class="td center edit" style=" width:10%;"><a href="<?php echo U('mailqueue_edit',array('m_id'=>$vo['m_id']));?>">修改</a></div>
        <div class="clear"></div>
    ...
    <div class="footso">
        <form action="?" method="get">
            <div class="sobox">
                ...
                <input name="key_type_cn" id="J_key_type_cn" type="hidden" value="<?php echo ((_I($_GET['key_type_cn']) != "")?(_I($_GET['key_type_cn'])):'邮件主题'); ?>"/>
                <input name="" type="submit" value="" class="sobtn"/>
                <div class="sotype" id="J_key_click"><?php echo ((_I($_GET['key_type_cn']) != "")?(_I($_GET['key_type_cn'])):'邮件主题'); ?></div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#ButSend").click(function () {
            ...
            var url = "<?php echo U('Ajax/send_mail_queue');?>";
        ...
        $("#ButDel").click(function () {
            ...
            var url = "<?php echo U('Ajax/delete_mail_queue');?>";
            $.post(url, data,function (result) {
                if(result.status == 1){
                    qsDialog.setContent(result.data);
                } else {
