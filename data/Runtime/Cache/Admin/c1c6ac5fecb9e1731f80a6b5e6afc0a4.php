...
<head>
	...
	<script>
		var URL = '/index.php/admin/config',
			SELF = '/index.php?m=admin&amp;c=config&amp;a=other',
...
<form action="<?php echo U('edit');?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
    ...
    <div class="form_main width200">
        <div class="fl">开启关键词URL编码：</div>
        <div class="fr">
            <div data-code="0,1" class="imgchecked_small <?php if(C('qscms_key_urlencode') == 1): ?>select<?php endif; ?>"><input name="key_urlencode" type="hidden" value="<?php echo C('qscms_key_urlencode');?>" /></div>
            <div class="note">（当关键词搜索出现乱码时，请开启此项）</div>
            <div class="clear"></div>
        </div>
        <div class="fl"></div>
        <div class="fr">
...
<form action="<?php echo U('edit');?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
    ...
    <div class="form_main width200">
        <div class="fl">薪资单位：</div>
        <div class="fr">
            <div class="imgradio">
                <input name="wage_unit" type="hidden" value="<?php echo C('qscms_wage_unit');?>">
                <div class="radio <?php if(C('qscms_wage_unit') == 1): ?>select<?php endif; ?>" data="1" title="K">K</div>
            <div class="radio <?php if(C('qscms_wage_unit') == 2): ?>select<?php endif; ?>" data="2" title="千">千</div>
        <label class="note-radio">（如：1.5K~3K ，8千5~1万）</label>
        <div class="clear"></div>
    </div>
    </div>
    <div class="fl"></div>
