...
<head>
	...
	<script>
		var URL = '/index.php/admin/crons',
			SELF = '/index.php?m=admin&amp;c=crons&amp;a=index',
...
<form id="form1" name="form1" method="post" action="<?php echo U('crons/delete');?>">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_black">
        <div class="td" style="width:55%;">
            <div class="left_padding striking"><input name="cronid[]" type="checkbox"  value="<?php echo ($vo["cronid"]); ?>" /><?php echo ($vo["name"]); ?>&nbsp;</div>
        </div>
        <div class="td center" style=" width:10%;">
            <?php if($vo['available'] == '1'): ?>可用<?php else: ?><span style="color:#999999">不可用</span><?php endif; ?>&nbsp;
        </div>
        <div class="td center" style=" width:10%;">
            <?php if($vo['admin_set'] == '1'): ?>内置<?php else: ?>自定义<?php endif; ?>&nbsp;
        </div>
        <div class="td center" style=" width:12%;"><?php if($cache[$vo['cronid']]): echo date('Y-m-d H:i',$cache[$vo['cronid']]['start']); else: ?>0<?php endif; ?>&nbsp;</div>
