...
class ImageGd{
    ...
    public function open($imgname){
        ...
        switch ($this->info['type']){
        	case 'bmp'://当前执行的操作为将成员加入黑名单，成功后它的反向操作为"恢复"
        		/* require_once 'BMP.class.php';
        		$bmp = new BMP();
        		$this->img = $bmp->imagecreatefrombmp($imgname);
        		$this->save($this->img,str_replace('bmp','jpg',$imgname)); */
        		$this->img = imagecreatefrombmp($imgname);
        		break;
        	case 'gif'://当前执行的操作为将小组成员设为正常用户，成功后反向操作为"加入黑名单"或"封禁"
    ...
    public function save($imgname, $type = null, $interlace = true){
        ...
        if('gif' == $type && !empty($this->gif)){
            ...
            $fun = "image{$type}";
            $ret = $type == 'jpeg' ? $fun($this->img, $imgname,100) : $fun($this->img, $imgname);
        }
    }
    /**
     * 返回图像宽度
