...
function contact_hide($data,$IsWhat = 2){
    if($IsWhat == 1){
        ...
        return  preg_replace('/(1[23456789]{1}[0-9])[0-9]{4}([0-9]{4})/i','$1****$2',$data);
    }elseif($IsWhat == 3){
...
function check_url($subsite_id){
    ...
    if(C('SUBSITE_VAL.s_id') != $subsite_id){
        if(C('qscms_subsite_method') == 1){
            if(false === $subsite = F('subsite_domain_list')) $subsite = D('Subsite')->get_subsite_domain();
            if(C('PLATFORM')=='mobile' && $subsite[$subsite_id]['s_m_domain']){
                $domain = $subsite[$subsite_id]['s_m_domain'];
                $url = strtolower($_SERVER['REQUEST_URI']);
                $url = str_replace(array('m=mobile&','mobile/'),'',$url);
                $url = 'http://'.$domain.$url;
            }else{
                $domain = $subsite[$subsite_id]['s_domain'];
...
function GetIpLookup($ip = ''){
    ...
    if($ip_info = cookie('ip_info_'.$ip)) return $ip_info;
    $res = @file_get_contents('http://ip.taobao.com/service/getIpInfo.php?ip=' . $ip);  
    if(empty($res)) return false;
    $json = json_decode($res, true);
    if(!$json['data']['ip']) return false;
    if($json['data']){
        $obj['ip'] = $ip;
        $obj['province'] = $json['data']['region'];
        $obj['city'] = $json['data']['city'];
        $obj['district'] = $json['data']['county'];
        cookie('ip_info_'.$ip,$obj);
    }else{  
        return false;  
    }  
    return $obj;
