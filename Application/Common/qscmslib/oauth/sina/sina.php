...
class sina_oauth
{
    ...
    public function __construct($setting) {
        if(C('PLATFORM') == 'mobile'){
            $this->redirect_uri = C('qscms_site_domain').C('qscms_site_dir').U('mobile/callback/oauth', array('mod'=>'sina'), '', '', true);
        }else{
            $this->redirect_uri = C('qscms_site_domain').C('qscms_site_dir').U('callback/oauth', array('mod'=>'sina'), '', '', true);
        }
