...
class store_transfer_showTag {
    ...
    public function run(){
        ...
        $subsite_id = get_jobs_subsite_id($val);
        $val['url']=url_rewrite('QS_storetransfer_show',array('id'=>$val['id']),$subsite_id);
        ...
        $val['img'] = M('StoretransferImg')->where(array('pid'=>$val['id'],'display'=>1,'audit'=>1))->select();
        foreach ($val['img'] as $key => $value) {
            $val['img'][$key]['img'] = attach($value['img'],'storetransfer');
        }
