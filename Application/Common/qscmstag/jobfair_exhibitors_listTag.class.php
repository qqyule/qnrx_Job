...
class jobfair_exhibitors_listTag {
    ...
    function __construct($options) {
        ...
        $this->tablename = C('DB_PREFIX').'jobfair_exhibitors';
		...
		$this->params['listpage']=isset($this->params['listpage'])?$this->params['listpage']:'QS_jobfairexhibitors';
		$this->params['dot']=isset($this->params['dot'])?$this->params['dot']:'';
    }
    public function run(){
    	if($this->params['paged']){
    		$count = M('JobfairExhibitors')->where($this->map);
	    	$total = $count->count();
        ...
        $result = M('JobfairExhibitors')->join($join)->where($this->map)->order($this->order)->limit($this->limit)->select();
        $list = array();
        foreach ($result as $key => $value) {
            ...
            if ($row['uid']>0)
            {
            $row['url'] =url_rewrite($this->params['showname'],array('id'=>$row['company_id']),$row['subsite_id']);
            }
            else
            {
            $row['url']="";
            ...
            foreach ($jobslist as $k => $v) {
                ...
                $row['jobslist'][$k]['jobs_url']= url_rewrite('QS_jobsshow',array('id'=>$v['id']),$subsite_id);
            }
			$list[] = $row;
        }
        /**
        ...
        $result = M('JobfairExhibitors')->join($join)->where($this->map)->order($this->order)->limit(7)->select();
        foreach ($result as $key => $value) {
            $row = $value;
