...
class jobfair_listTag {
    ...
    public function run(){
        ...
        foreach ($result as $key => $value) {
            ...
            $row['exhibitorsurl'] = url_rewrite($this->params['exhibitorspage'],array('id'=>$row['id']),$row['subsite_id']);  
            $row['booth_url'] = url_rewrite('QS_jobfair_booth',array('id'=>$row['id']),$row['subsite_id']);  
            // 1预定中 0结束预定
