...
class careerTag {
    ...
    function __construct($options) {
        ...
        if(C('SUBSITE_VAL')) $this->map['subsite_id'] = C('SUBSITE_VAL.s_id');
        ...
        $this->params['showname']=isset($this->params['showname'])?$this->params['showname']:'QS_career_show';
        $this->params['listpage']=isset($this->params['listpage'])?$this->params['listpage']:'QS_career_list';
        $this->params['dot']=isset($this->params['dot'])?$this->params['dot']:'';
    }
    public function run(){
        if($this->params['paged']){
            $count = M('Career')->where($this->map);
            $total = $count->count();
        ...
        $result = M('Career')->where($this->map)->order($this->order)->limit($this->limit)->select();
        $list = array();
        foreach ($result as $key => $value) {
            ...
            {
                $row['url'] = strpos($row['is_url'],'http://') ? $row['is_url'] : 'http://'.$row['is_url'];
            }
            else
            {
                $row['url'] = url_rewrite($this->params['showname'],array('id'=>$row['id']),$row['subsite_id']);
            }
