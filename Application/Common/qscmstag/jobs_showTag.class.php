    ...
    public function run(){
       ...
       if(!C('visitor.uid')){
        ...
        if(C('visitor.utype') == 2){
            ...
            $view_log = M('ViewJobs')->where(array('uid'=>C('visitor.uid'),'jobsid'=>$val['company_id']))->find();
            if($view_log){
        ...
        if(C('qscms_apply_jobs_contact')){
                $apply = D('PersonalJobsApply')->where(array('personal_uid'=>C('visitor.uid'),'company_id'=>$val['company_id']))->find();
                if($apply){
                    if($val['contact']['telephone']){
                        $val['contact']['telephone'] = $val['contact']['telephone'];
                    }else{
        ...
        if(C('qscms_contact_img_com') == 2){
            $val['contact']['telephone'] = '<img src="'.C('qscms_site_domain').U('Home/Qrcode/get_font_img',array('str'=>encrypt($val['contact']['telephone'],C('PWDHASH'))),'','',true).'"/>';
