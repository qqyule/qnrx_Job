...
class senior_jobfair_showTag {
    ...
    public function run(){
        ...
        $val['url'] = url_rewrite("QS_seniorjobfairshow",array('id'=>$val['id']),$val['subsite_id']);
        $val['url'] = C('apply.Subsite') ? $val['url'] : C('qscms_site_domain') . $val['url'];
        $val['exhibitorsurl'] = url_rewrite($this->params['exhibitorspage'],array('id'=>$val['id']),$val['subsite_id']); 
        $val['booth_url'] = url_rewrite('QS_seniorjobfair_booth',array('id'=>$val['id']),$val['subsite_id']);  
