...
class news_showTag {
    ...
    public function run(){
        ...
        $fields = C('apply.Subsite') ? 'id,type_id,title,subsite_id' : 'id,type_id,title';
        $where['type_id'] = $val['type_id'];
        C('apply.Subsite') && $where['subsite_id'] = $val['subsite_id'];
        $list = M('Article')->where($where)->order('article_order desc, addtime desc')->field($fields)->select();
        $current = -1;
        foreach ($list as $k => $v){
            if ($v['id'] == $val['id']){
                $current = $k;
