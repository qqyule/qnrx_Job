...
use Think\Model;
class AdvResumeImgModel extends Model{
	protected $_validate = array(
		array('resume_id,img','identicalNull','',1,'callback'),
		array('resume_id','identicalEnum','',1,'callback'),
	...
	public function save_resume_img($data,$user){
		...
		if($data['id']){
			if(false === $this->where(array('id'=>$data['id'],'resume_id'=>$data['resume_id']))->save()) return array('state'=>0,'error'=>'附件修改失败，请重新操作！');
			$id = $data['id'];
		}else{
		...
		//写入会员日志
		write_members_log($user,'resume','添加高级简历附件（简历id：'.$data['resume_id'].'）',false,array('resume_id'=>$data['resume_id']));
		return array('state'=>1,'id'=>$id);
	}
	public function get_resume_img($pid)
	{  
		$list = $this->where(array('resume_id'=>$pid))->select();
		foreach ($list as $key => $value) {
			$list[$key]['img'] = attach($value['img'],'top_resume_img');
		}
		return $list;
	}
	/**
