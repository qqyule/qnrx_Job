...
use Think\Model;
class ResumeTrainingModel extends Model{
	protected $_validate = array(
		...
		//字段数字验证
		array('uid,pid,startyear,startmonth,endyear,endmonth,todate','identicalEnum','{%resume_training_end_time_enum}',1,'callback'),
	);
	/**
	 * [培训结束时间验证]
	 * 培训结束年份+月份、至今必选一项
	...
	*/
	public function add_resume_training($data,$user)
	{
		...
		//写入会员日志
		write_members_log($user,'resume','添加简历培训经历（简历id：'.$data['pid'].'）',false,array('resume_id'=>$data['pid']));
		$resume_mod = D('Resume');
