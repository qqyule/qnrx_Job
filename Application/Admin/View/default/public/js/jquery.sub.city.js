...
!function($) {
    ...
    $('[data-toggle="funCityModal"]').live('click', function() {
        ...
        for (var i = 0; i < QS_city_parent.length; i++) {
            cateParentHtml += '<option value="' + QS_city_parent[i].split(',')[0] + '" title="'+QS_city_parent[i].split(',')[1]+'">' + QS_city_parent[i].split(',')[1] + '</option>';
        }
        ...
        if (recoverVal.length) {
            ...
            if (firstKeepHtml) {
                ...
                getCurrentHtml($('.categorySelect').eq(0).find('option:selected').val(), $('.categorySelect').eq(0).find('option:selected').text());
            }
        }
        // 父级选择
        $('.categorySelect').die().live('change', function() {
            ...
            var currentIndex = $('.categorySelect').index($(this));
            $(this).nextAll().remove();
            if (QS_city[currentVal]) {
                // 判断是否是不限
                if (!($(this).find('option:selected').text() == '不限')) {
                    // 判断是否是倒数第二级
                    if (getSubCateLevel(currentVal, '').split('.').length > 1) {
                        getCurrentHtml(currentVal, currentText);
                    }
                }
            } else {
        ...
        // 生成当前列表
        function getCurrentHtml(dataId, dataText) {
            var dataArr = QS_city[dataId].split('`');
            var cateSubHtml = '<select class="categorySelect">';
            cateSubHtml += '<option value="'+dataId+'" title="'+dataText+'">不限</option>';
            for(j = 0;j < dataArr.length; j++){
                cateSubHtml += '<option value="'+dataArr[j].split(',')[0]+'" title="'+dataArr[j].split(',')[1]+'">'+dataArr[j].split(',')[1]+'</option>';
            }
            cateSubHtml += '</select>';
        ...
        function getSubCateHtml(currendId, dataArr) {
            ...
            for(i = 0;i < subCateLevelArr.length-1; i++) {
                ...
                cateSubHtml += '<option value="'+currendId+'" title="'+getName(currendId)+'">不限</option>';
                var citySubArr = QS_city[subCateLevelArr[i]].split('`');
                for(j = 0;j < citySubArr.length; j++){
                    cateSubHtml += '<option value="'+citySubArr[j].split(',')[0]+'" title="'+citySubArr[j].split(',')[1]+'">'+citySubArr[j].split(',')[1]+'</option>';
                }
                cateSubHtml += '</select>';
            }
            $('#selectCategoryBox').append(cateSubHtml);
        ...
        $('#J_btn_yes_city').die().live('click', function() {
            ...
            var currentText = $('.categorySelect:last').find('option:selected').attr('title');
            $('.J_resuletitle_city').val(currentText);
            $('.J_resultcode_city').val(currentVal);
            removeModal();
        })
    })
