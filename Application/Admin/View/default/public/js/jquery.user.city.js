 ...
 * ============================================================ */
!function($) {
    var backdropLayerTplNewCity = '<div class="modal_backdrop fade"></div>';
    var htmlLayerTplNewCity = ['<div class="modal qs-category-unlimited">', '<div class="modal_dialog">', '<div class="modal_content pie_about">', '<div class="modal_header">', '<span class="title J_modal_title"></span>', '<span class="max_remind J_modal_max"></span>', '<a href="javascript:;" class="close J_dismiss_modal"></a>', '</div>', '<div class="modal_body pd0">', '<div class="listed_group" id="J_listed_group">', '<div class="left_text">已选择：</div>', '<div class="center_text" id="J_listed_content"></div>', '<a href="javascript:;" class="right_text" id="J_listed_clear">清空</a>', '<div class="clear"></div>', '</div>', '<div class="J_modal_content"></div>', '</div>', '<div class="modal_footer">', '<div class="res_add_but">', '<div class="butlist">', '<div class="btn_blue J_hoverbut btn_100_38 J_btn_yes">确 定</div>', '</div>', '<div class="butlist">', '<div class="btn_lightgray J_hoverbut btn_100_38 J_dismiss_modal J_btn_cancel">取 消</div>', '</div>', '<div class="clear"></div>', '</div>', '</div>', '<input type="hidden" class="J_btnload" />', '</div>', '</div>', '</div>'].join('');
    // 处理需要复原的数据
    ...
    $('[data-toggle="funCityModal"]').live('click', function() {
        ...
        var defaultKeyArr = '';
        var recoverVal = $('.J_resultcode_city').val();
        var recoverKeepVal = $('.J_resultcode_city').attr('keep');
            ...
            if (multipleValue) {
                ...
                if (!$(this).hasClass('noLimit')) {
                    if ($('.categorySelect').length >= 1) {
                        $('.J_resultcode_city').val(thisCode.split('.')[thisCode.split('.').length-1]);
                    } else {
                        $('.J_resultcode_city').val(thisCode);
                    }
                } else {
                    if ($('.categorySelect').length >= 1) {
                        $('.J_resultcode_city').val(thisCode.split('.')[thisCode.split('.').length-1]);
                ...
                $('.J_resultcode_city').attr('keep', thisCode);
                $('.J_resuletitle_city').val(thisText);
                $('.J_resuletitle_city').attr('title', thisText);
                removeModal();
            }
        })
        // 根据id获取对应文字
        ...
        $('#J_btn_yes_city').die().live('click', function() {
            ...
            $('.J_resultcode_city').attr('keep', keepArray.join(','));
            $('.J_resuletitle_city').val(textArray.length ? textArray.join('+') : '请选择');
            $('.J_resuletitle_city').attr('title', textArray.length ? textArray.join('+') : '请选择');
            removeModal();
        })
        // 同步
    ...
    function prepareModal(titleValue, multipleValue, maximumValue) {
        ...
        $(backdropLayerTplNewCity).appendTo($(document.body));
        if (ie6) {
            $('.modal_backdrop').css("height", $(document).height());
        }
        $(htmlLayerTplNewCity).appendTo($(document.body));
