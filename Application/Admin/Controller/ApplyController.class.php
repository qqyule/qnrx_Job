...
class ApplyController extends BackendController{
	...
	protected $_rollback = array(
        'Home'         => array('4.2.56','4.2.59','4.2.95'),
        'Mobile'       => array('4.2.49'),
        'Subsite'      => array('4.2.4'),
    ...
    public function index(){
        ...
        $info = $model->query("SHOW COLUMNS FROM `".C('DB_PREFIX')."members` like 'secretkey'");
        $config = $model->query("SELECT * FROM `".C('DB_PREFIX')."config` WHERE `name` = 'PWD_ENCRYPT_METHOD'");
        $this->assign('is_shift_4295',count($info) && $config?1:0);
    ...
    public function setup(){
        ...
        if(!IS_POST){
            ...
            //检测版本号是否可以安装
            $mod_H = $apply['Home']['version'];
            $mod_H =  explode('.', $mod_H);
            $vH = $mod_H[0] * 1000000 + $mod_H[1] * 10000 + $mod_H[2];
            foreach($this->_setup as $key=>$val){
                if($key <= $vH){
                    if($modV = $val[$mod]){
                        $modV =  explode('.', $modV);
    ...
    public function updater_auth(){
        ...
        if($time != $time_ori || !$_cache = S('_apply_updater_cache')){
            ...
            if(!IS_POST){
                ...
                $this->_show_process('--------------------------------------------------------------------------------------------------------------------');
                $this->_show_process('删除缓存文件...');
                $this->Clearcache();
                $this->_show_process('删除缓存文件成功');
    ...
    protected function Clearcache(){
        ...
        if (is_writable(QSCMS_DATA_PATH.'static')){           
            rmdirs(QSCMS_DATA_PATH.'static');
        }
        if (is_writable(STATISTICS_PATH)){           
            rmdirs(STATISTICS_PATH);
        }
        if (is_writable(QSCMS_DATA_PATH.'wxpay')){           
            rmdirs(QSCMS_DATA_PATH.'wxpay');
        }
        if (is_writable(QSCMS_DATA_PATH.'html')){           
