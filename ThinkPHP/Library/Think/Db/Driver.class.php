...
abstract class Driver {
    ...
    // 数据库表达式
    protected $exp = array('eq'=>'=','neq'=>'<>','gt'=>'>','egt'=>'>=','lt'=>'<','elt'=>'<=','notlike'=>'NOT LIKE','like'=>'LIKE','in'=>'IN','notin'=>'NOT IN','not in'=>'NOT IN','between'=>'BETWEEN','not between'=>'NOT BETWEEN','notbetween'=>'NOT BETWEEN','is'=>'IS','is not'=>'IS NOT','match'=>'MATCH','match_mode'=>'MATCH','match_with'=>'MATCH');
    // 查询表达式
     ...
     * @return mixed
     */
    public function query($str,$fetchSql=false,$s = false) {
     ...
     * @return mixed
     */
    public function execute($str,$fetchSql=false,$s=false) {
    ...
    protected function debug($start) {
        if($start) {
            ...
            // 记录操作结束时间
            G('queryEndTime');
            $t = G('queryStartTime','queryEndTime');
            $this->config['debug'] && trace($this->queryStr.' [ RunTime:'.$t.'s ]','','SQL');
            if(0.2 <= $t = abs($t)) Log::write($this->queryStr.' [RunTime:'.$t.'s] [IP:'.get_client_ip().'] [AddTime:'.time().'] [qscms]','SQL','File',SQL_LOG_PATH.date('y_m_d').'.log');
        }
    }
