...
function U($url='',$vars='',$suffix=true,$domain=false,$type=false,$module_type=false) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由
            ...
            if(C('apply.Subsite') && MODULE_NAME != 'Admin' && CONTROLLER_NAME != 'Admin' && C('SUBSITE_DOMAIN')){
                $subsite_list = D('Subsite')->get_subsite_cache();
                $temp = str_replace(array('http://','https://'),'',C('SUBSITE_DOMAIN'));
                if($subsite_list[$temp] && $temp == $subsite_list[$temp]['s_m_domain']){
                    $module = '';
                    unset($var[$varModule]);
                }
            }
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(preg_match('/com.cn|net.cn|gov.cn|org.cn$/',$_SERVER['HTTP_HOST']) === 1){
            $domain = array_slice(explode('.', $_SERVER['HTTP_HOST']), -3, 3);
        }else{
            $domain = array_slice(explode('.', $_SERVER['HTTP_HOST']), -2, 2);
        }
        $domain = '.'.implode('.',$domain);
        if(isset($name['domain']))          {
            ini_set('session.cookie_domain', $domain);
            $cookie_domain = C('COOKIE_DOMAIN');
            //if(strpos($cookie_domain,$domain)) C('COOKIE_DOMAIN',$domain);
